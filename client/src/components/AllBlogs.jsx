...
const AllBlogs = ({ isAdmin = true }) => {
  ...
  useEffect(() => {
    const getBlogs = async () => {
      ...
      try {
        const query = isAdmin
          ? `/api/blog/get-all-blogs?page=1`
          : `/api/blog/get-all-blogs?userId=${user._id}&page=1`;
  ...
  const fetchBlogs = async (page = 2) => {
    try {
      const query = isAdmin
        ? `/api/blog/get-all-blogs?page=${page}`
  ...
  return (
    <>
      <div
        className={`transition-all border my-2 mx-2 rounded-md w-full md:mx-5 overflow-x-auto scrollbar ${
          theme === "dark"
            ? "border-zinc-700 bg-gray-900"
                ...
                >
                  ...
                  <Table.Cell className="flex justify-center">
                    <NavLink className="text-center" to={`/blog/${data.slug}`}>
                      <img
                        src={`${BACKEND_URL}${data.blogImgFile}`}
                        alt="blogImage"
